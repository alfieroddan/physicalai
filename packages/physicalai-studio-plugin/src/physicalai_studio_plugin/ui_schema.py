    ...
    def validate_items(  # noqa: C901, PLR0912, PLR0915
        ...
        for index, item in enumerate(items):
            ...
            if kind == "info":
                ...
                if "variant" in item and item["variant"] not in {"info", "warning"}:
                    error(item_path, "info item variant must be 'info' or 'warning'")
                continue
            ...
            if kind == "connection":
                ...
                for option_name in ("device_discovery", "identify", "manual_entry", "required"):
                    if option_name in item and not isinstance(item[option_name], bool):
                        error(item_path, f"connection item {option_name} must be a boolean")
                continue
        ...
        if isinstance(properties, dict):  # noqa: PLR1702
            for field_name, field_schema in properties.items():
                ...
                if isinstance(resolved, dict):
                    field_ui = resolved.get("x-physicalai-ui")
                    if field_ui is not None and not isinstance(field_ui, (dict, list)):
                        error(f"{path}.properties.{field_name}.x-physicalai-ui", "must be an object or item list")
                    if isinstance(field_ui, dict):
                        for option_name in ("required", "advanced_configuration"):
                            if option_name in field_ui and not isinstance(field_ui[option_name], bool):
                                error(
                                    f"{path}.properties.{field_name}.x-physicalai-ui",
